...
目前能力合同明确列出 7 个 stable 窄检测家族、0 个 planned 检测 adapter 和 6 项 agent-guided 方法；证据/报告与分发能力不计入检测覆盖。另有 3 个固定 commit 的普通开源项目旅程，保留零 finding、误报关闭和 unknown 结果，未探测线上实例。
