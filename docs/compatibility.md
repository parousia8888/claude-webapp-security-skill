...
Lifecycle commands do not fetch remote code. `install` copies the checkout or extracted release
that executes it. To upgrade, download and verify the newer release, then run that payload's
`scripts/webapp-security.mjs upgrade`. Replacement and removal require a recognized install marker
or the documented legacy Skill identity; unknown paths are left untouched. Native Windows and
PowerShell launchers remain unsupported.
