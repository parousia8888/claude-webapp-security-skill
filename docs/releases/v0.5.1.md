...
The published `SHA256SUMS`, manifest source commit, `git rev-parse 'v0.5.1^{}'` and GitHub provenance
were verified before the immutable asset digests were added to the installer. The online verified
installation required and passed attestation verification. The SSH-signed `v1` alias was then moved
with a guarded lease to the same source; public consumer run `31773652542` passed passive-boundary
and authorization-rejection checks before the promotion was recorded complete.
