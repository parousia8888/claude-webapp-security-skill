...
The published `SHA256SUMS`, manifest source commit, `git rev-parse 'v0.5.2^{}'`, GitHub-recorded
asset digests and provenance were verified before the immutable asset digests were added to the
installer. The mutable `v1` alias moves only after the verified installation path and public
consumer workflow pass.
