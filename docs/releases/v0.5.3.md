# v0.5.3 release evidence

Scope, audit, harden, and retest web projects with AI coding agents and reproducible evidence.

Web App Security Skill v0.5.3 reduces first-run friction and source-review noise. It adds npm/npx
and Claude Code plugin distribution, public current limitations, Git diff-scoped source audit and a
reproducible planted pattern benchmark. The stable detection-rule count does not change.
...
digests and provenance were verified before immutable asset digests were added to the installer.
The signed `v1` tag object `e0f26194119e541b1ac108a17ea6ee2f2f7699c5` was promoted with an exact
lease to source commit `621e0bc2ad044f9390fa9d567bf4b9fca138a959`; public consumer workflow
`31941632774` passed both passive behavior and authorization rejection.

`web-app-security-skill@0.5.3` was published to npm from a detached worktree at the same source
commit after account write 2FA was enabled. The pre-publish package and public registry metadata
match at shasum `7fb87e4915ce654c3f9e4cb03f7f01688a8e9f03` and integrity
`sha512-VgrPSSmQ5HsW6neJsp4Kcc8R5YNcecCXohvCSXwSXfG6TEQ+3pYgK+WGmT7gV1E3OSMRcGO4L70uof4Hv9yRRw==`.
A fresh npm cache ran `npx --yes web-app-security-skill@0.5.3 audit . --fail-on never` against an
isolated local fixture and produced the expected v3 report with no network target. This npm
publication does not claim a separate npm provenance attestation; the GitHub release-asset
provenance described above is a distinct trust path.
